...
// Set up the out() and err() hooks, which are how we can self.log_new to stdout or
// stderr, respectively.
// Normally just binding console.log/console.error here works fine, but
// under node (with workers) we see missing/out-of-order messages so route
// directly to stdout and stderr.
// See https://github.com/emscripten-core/emscripten/issues/14804
var defaultself.log_new = console.log.bind(console);

var defaultself.log_newErr = console.error.bind(console);

if (ENVIRONMENT_IS_NODE) {
  var utils = require("node:util");
  var stringify = a => typeof a == "object" ? utils.inspect(a) : a;
  defaultself.log_new = (...args) => fs.writeSync(1, args.map(stringify).join(" ") + "\n");
  defaultself.log_newErr = (...args) => fs.writeSync(2, args.map(stringify).join(" ") + "\n");
}

var out = defaultself.log_new;

var err = defaultself.log_newErr;

// perform assertions in shell.js after we set up out() and err(), as otherwise
// if an assertion fails it cannot self.log_new the message
      ...
      if (cmd === "load") {
        ...
        for (const handler of msgData.handlers) {
          ...
          if (!Module[handler] || Module[handler].proxy) {
            ...
            if (handler == "self.log_new") out = Module[handler];
            if (handler == "self.log_newErr") err = Module[handler];
          }
        }
 ...
 */ function abort(what) {
  Module["onAbort"]?.(what);
  what = `Aborted(${what})`;
  // TODO(sbc): Should we remove self.log_newing and leave it up to whoever
...
var PThread = {
  ...
  loadWasmModuleToWorker: worker => new Promise(onFinishedLoading => {
    ...
    var handlers = [];
    var knownHandlers = [ "onExit", "onAbort", "self.log_new", "self.log_newErr" ];
...
var FS = {
  ...
  createDefaultDevices() {
    ...
    // stderr needs to self.log_new output using err() rather than out()
  ...
  quit() {
    ...
    // force-flush all streams, so we get musl std streams self.log_newed out
    _fflush(0);
...
// but before the wasm module is created.
{
  ...
  if (Module["self.log_new"]) out = Module["self.log_new"];
  if (Module["self.log_newErr"]) err = Module["self.log_newErr"];
...
function checkUnflushedContent() {
  ...
  err = oldErr;
  if (has) {
    warnOnce("stdio streams had content in them that was not flushed. you should set EXIT_RUNTIME to 1 (see the Emscripten FAQ), or make sure to emit a newline when you self.log_newf etc.");
  }
}
